...
export const getItemMaxScore = (item: QtiItem): number => {
  return item.rubric.reduce(
    (sum, criterion) => sum + (Number.isFinite(criterion.points) ? criterion.points : 0),
    0,
  )
}
...
export const getRubricScore = (item: QtiItem, rubricOutcomes: Record<number, boolean>): number => {
  return item.rubric.reduce(
    (sum, criterion) => sum + (rubricOutcomes[criterion.index] ? criterion.points : 0),
    0,
  )
}
