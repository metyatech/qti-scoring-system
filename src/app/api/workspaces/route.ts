...
const stripSharedRootSegment = (value: string, rootSegment: string | null) => {
  ...
  return value.startsWith(`${rootSegment}/`)
    ? value.slice(rootSegment.length + 1)
    : value;
};
...
export async function POST(request: NextRequest) {
  try {
    ...
    const assessmentInputs = form
      .getAll("assessmentFiles")
    ...
    const assessmentTestEntries = assessmentEntries.filter(
      (entry) =>
        path.posix.basename(entry.safePath) === "assessment-test.qti.xml"
    );
