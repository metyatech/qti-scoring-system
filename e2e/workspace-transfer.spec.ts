...
import { cleanupTrackedWorkspaces, createWorkspace, deleteWorkspace } from './utils/workspace'

test.afterEach(async ({ page }) => {
  await cleanupTrackedWorkspaces(page.request)
})

test('workspace export can be imported from the home screen', async ({ page }) => {
  ...
  try {
    ...
    const tempDir = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'workspace-export-'))
    ...
    await expect(page.getByRole('heading', { name: 'E2E Transfer' }).first()).toBeVisible()
